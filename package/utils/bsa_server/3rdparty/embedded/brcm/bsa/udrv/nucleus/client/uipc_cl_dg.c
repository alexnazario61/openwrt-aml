...
{
    BOOLEAN uipc_cl_status = TRUE;
    UINT8 *p_buffer = NULL;
    /* Get to the data buffer */
    p_buffer = ((UINT8 *)(p_msg + 1)) + p_msg->offset;

    uipc_cl_status = uipc_channel_send(uipc_cl_dg_tx_desc, p_buffer, p_msg->len);
    GKI_freebuf(p_msg);

    if (FALSE == uipc_cl_status)
    {
        APPL_TRACE_ERROR0("uipc_cl_dg_send_buf cannot send data");
    }

    return uipc_cl_status;
