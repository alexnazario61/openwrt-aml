...
APPL_TRACE_DEBUG0("uipc_sv_control_open enter");
    ...
    uipc_sv_ctl_rx_desc = uipc_channel_open(channel_id, uipc_sv_control_cback, UIPC_SV_RX_CONTROL_MODE);
    if (uipc_sv_ctl_rx_desc == UIPC_CHANNEL_BAD_DESC)
    {
        APPL_TRACE_ERROR0("uipc_sv_control_open fails to open Rx control channel");
        return FALSE;
    }
    /* Open a channel in Tx/write direction */
    uipc_sv_ctl_tx_desc = uipc_channel_open(channel_id, NULL, UIPC_SV_TX_CONTROL_MODE);
    if (uipc_sv_ctl_tx_desc == UIPC_CHANNEL_BAD_DESC)
    {
        APPL_TRACE_ERROR0("uipc_sv_control_open fails to open Tx control channel");
        return FALSE;
    }
...
static void uipc_sv_control_cback(BT_HDR *p_msg)
{
    ...
    if (p_msg->event == UIPC_CHANNEL_OPEN)
    {
        ...
        {
            APPL_TRACE_ERROR0("ERROR uipc_sv_control_cback NULL Callback !!!!!");
        }
    }
    ...
    {
        APPL_TRACE_ERROR1("uipc_sv_control_cback bad event:%d received from UIPC",
                p_msg->event);
    }
...
/*    scru_dump_hex(p_buffer, "Data", length, 0, 0);*/
    ...
    do
    {
        ...
        status = bsa_strm_dec(&uipc_sv_rx_control_stream[client_num],
                &p_buffer, &length);
        ...
        if (status)
        {
                    ...
                    sizeof(tBSA_SV_CONTROL_CLIENT_MSG))) != NULL)
            {
                ...
                {
                    APPL_TRACE_ERROR0("ERROR uipc_sv_control_rx_data NULL Callback !!!!!");
                }
            }
            else
            {
