...
#define UIPC_CHANNEL_DESC_MAX   10
...
#define UIPC_THREAD_NAME "UIPC-READ-THREAD"
        ...
        UIPC_CHANNEL_MODE mode)
{
...
#ifdef DEBUG_UIPC_CHANNEL
    APPL_TRACE_DEBUG2("uipc_channel_open channel_id:%d mode:%x", channel_id, mode);
#endif

    /* Check Read/Write mode */
    if ((mode & UIPC_CHANNEL_MODE_READ) && (mode & UIPC_CHANNEL_MODE_WRITE))
    {
    ...
    if ((mode & UIPC_CHANNEL_MODE_READ) && ((mode & UIPC_CHANNEL_MODE_RX_MASK) == 0))
    {
    ...
    if ((mode & UIPC_CHANNEL_MODE_WRITE) && ((mode & UIPC_CHANNEL_MODE_TX_MASK) == 0))
    {
    ...
    if (mode & UIPC_CHANNEL_MODE_CREATE)
    {
        ...
        for (index = 0; index < UIPC_CHANNEL_DESC_MAX; index++)
        {
            ...
            if (uipc_channel_desc_cb[index].nb_open == 0)
            {
                ...
                status = NU_Create_Event_Group (&uipc_channel_desc_cb[index].OSEvtGrp,
                    UIPC_THREAD_NAME);
...
#endif
        /* Allocate memory for UIPC-Read-thread */
        uipc_channel_desc_cb[index].p_thread_buff = GKI_os_malloc(16 * 1024);
        /* Allocate memory for task 1 */
        if (uipc_thread_create(uipc_channel_read_task, (UINT8 *)UIPC_THREAD_NAME,
                uipc_channel_desc_cb[index].p_thread_buff, 16 * 1024,
                &uipc_channel_desc_cb[index].read_thread, (void *) index) < 0)
        {
            uipc_channel_desc_cb[index].nb_open -= 1;
            return UIPC_CHANNEL_BAD_DESC;
        }
    ...
    {
        APPL_TRACE_DEBUG1("******* uipc_channel_open() Both ends of UIPC(%d) Connected! *****",
                channel_id);
...
#endif
        NU_Delete_Task(&uipc_channel_desc_cb[channel_desc].read_thread);
        GKI_os_free(uipc_channel_desc_cb[channel_desc].p_thread_buff);
 ...
 **
 *******************************************************************************/
BOOLEAN uipc_channel_send(tUIPC_CHANNEL_DESC channel_desc, UINT8 *buffer, INT16 length)
{
    BT_HDR *p_msg;
    ...
    {
        memset(p_msg, 0, sizeof(BT_HDR) + length);
#ifdef DEBUG_UIPC_CHANNEL
        ...
        /* Below channel id is used for DG Send data */
        if ((uipc_channel_desc_cb[channel_desc].channel_id >= UIPC_CH_ID_DG_FIRST) &&
                (uipc_channel_desc_cb[channel_desc].channel_id <= UIPC_CH_ID_DG_LAST))
        {
            p_msg->layer_specific = uipc_channel_desc_cb[channel_desc].channel_id;
        }
        else
        {
            p_msg->layer_specific = 0; /* This channel id is used for general Dm send data */
        }
        ...
        status = NU_Set_Events (&uipc_channel_desc_cb[channel_desc].OSEvtGrp,
            (UNSIGNED)msg_buf, NU_OR);

        /* Determine if the message was sent successfully.  */
        if (status == NU_SUCCESS) {
#ifdef DEBUG_UIPC_CHANNEL
            APPL_TRACE_DEBUG2("UIPC_TASK (channel:%d) : Send >>> %u", channel_desc, msg_buf);
#endif
        }
        else
        {
            APPL_TRACE_ERROR1("uipc_channel_send fail : %d", status);
        }
        ret_code = TRUE;
    }
    else
 ...
 **
 *******************************************************************************/
static void uipc_channel_read_task(UNSIGNED i, void *arg)
{
...
#ifdef DEBUG_UIPC_CHANNEL
        APPL_TRACE_DEBUG0("uipc_channel_read_task Waiting for channel event queue");
#endif
        status = NU_Retrieve_Events (&p_channel->OSEvtGrp, (UNSIGNED) 0xFFFF, NU_OR_CONSUME,
                (UNSIGNED *)&temp, 2000);

        if(status == NU_SUCCESS)
        {
#ifdef DEBUG_UIPC_CHANNEL
            APPL_TRACE_DEBUG2("uipc_channel_read_task msg_size:%d for channel:%d returns",
                msg_size, channel_desc);
            APPL_TRACE_DEBUG2("UIPC_TASK(channel:%d) : Received <<< %u", channel_desc, temp);
#endif
            ...
            {
#ifdef DEBUG_UIPC_CHANNEL
                APPL_TRACE_DEBUG3("uipc_channel_read_task event:%d len:%d layer_spec:%x",
                    p_msg->event, p_msg->len, p_msg->layer_specific);
                /*scru_dump_hex((UINT8 *)(p_msg + 1), "UIPC Data Rx",
                p_msg->len >= 32 ? 32 : p_msg->len, 0, 0);*/
#endif
                ...
                {
                    APPL_TRACE_ERROR1("uipc_msg_queue_read_task no callback for desc:%d",
                        channel_desc);
    ...
    APPL_TRACE_DEBUG1("uipc_channel_read_task channel:%d Read task exits", channel_desc);
}
