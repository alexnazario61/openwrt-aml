...
ifeq ($(INITRAMFS_BIN),)
  define Image/mkfs/cpiogz
	( cd $(TARGET_DIR); cat $(INITRAMFS) | cpio -o -H newc | gzip -9 >$(BIN_DIR)/$(IMG_PREFIX)-rootfs.cpio.gz )
	$(LINUX_DIR)/mkbootimg --kernel $(KDIR)/uImage --ramdisk $(BIN_DIR)/$(IMG_PREFIX)-rootfs.cpio.gz --second $(KDIR)/$(subst ",,$($(PROFILE)_KERNELDTS).dtb) --output $(BIN_DIR)/boot.img
  endef
else
  define Image/mkfs/cpiogz
	$(CP) $(INITRAMFS_BIN) $(BIN_DIR)/$(IMG_PREFIX)-rootfs.cpio.gz
	$(LINUX_DIR)/mkbootimg --kernel $(KDIR)/uImage --ramdisk $(BIN_DIR)/$(IMG_PREFIX)-rootfs.cpio.gz --second $(KDIR)/$(subst ",,$($(PROFILE)_KERNELDTS).dtb) --output $(BIN_DIR)/boot.img
  endef
endif
endif
